...
/*
class check1 {
    var isSitting: String
    init(isSitting: String) {
...
class PoseImageView: UIImageView {
  ///  var isSitting = "yay"
    ...
    /*
    @Published var a = check2().isSitting
    func idk(pose: PoseFinder.Pose) -> String {
        //var pose: [Pose]
        //let pose = Pose()
...
class PoseImageView1 {
    var a = check2().isSitting
    ...
    func show(poses: [Pose]) {
        ...
        for (pose in poses) {
    //let LAnk2Hip = pose[.leftAnkle].position.distance(to:pose[.leftHip].position)
    //let aa = String(LAnk2Hip)
    ///var LHip2Knee = pose[.leftHip].position.distance(to:pose[.leftKnee].position)
  ///  if LAnk2Hip == LAnk2Hip {
            isSitting = "bye"
    }
